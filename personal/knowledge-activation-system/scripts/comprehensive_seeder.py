...
CATEGORIES = {
    ...
    # =============================================================================
    # PHASE 1: CRITICAL GAPS (New Categories)
    # =============================================================================
    "ai-agents": {
        "namespace": "agents",
        "topics": [
            ("LangGraph Fundamentals", ["langgraph", "agents"], "StateGraph, nodes, edges, state management, conditional edges"),
            ("LangGraph Multi-Agent", ["langgraph", "multi-agent"], "supervisor patterns, agent handoffs, shared state"),
            ("LangGraph Tool Use", ["langgraph", "tools"], "ToolNode, tool calling, structured outputs, tool errors"),
            ("LangGraph Memory", ["langgraph", "memory"], "checkpointers, persistence, conversation history, state snapshots"),
            ("CrewAI Framework", ["crewai", "agents"], "agents, tasks, crews, sequential and hierarchical processes"),
            ("CrewAI Tools", ["crewai", "tools"], "custom tools, tool delegation, built-in tools"),
            ("AutoGen Conversations", ["autogen", "agents"], "conversable agents, group chat, code execution"),
            ("AutoGen Patterns", ["autogen", "patterns"], "two-agent, group chat, nested chats, teachable agents"),
            ("ReAct Pattern", ["react", "agents"], "reasoning, action, observation loops, implementation"),
            ("ReWOO Pattern", ["rewoo", "agents"], "plan-then-execute, tool planning, efficiency"),
            ("Agent Tool Use Claude", ["claude", "tools"], "tool definitions, tool_use blocks, structured responses"),
            ("Agent Tool Use OpenAI", ["openai", "functions"], "function calling, parallel tools, JSON mode"),
            ("Agent Memory Systems", ["agents", "memory"], "short-term, long-term, episodic, semantic memory"),
            ("Agent Orchestration", ["agents", "orchestration"], "routing, delegation, task decomposition"),
            ("Agent Evaluation", ["agents", "evaluation"], "task completion, tool accuracy, reasoning quality"),
            ("Multi-Agent Coordination", ["multi-agent", "coordination"], "communication protocols, shared state, conflict resolution"),
            ("Agent Debugging", ["agents", "debugging"], "trace visualization, step-through, logging strategies"),
            ("Agent Safety", ["agents", "safety"], "guardrails, output validation, sandboxing, rate limits"),
            ("Swarm Intelligence", ["swarm", "agents"], "collective behavior, emergent patterns, distributed agents"),
            ("Agent Benchmarks", ["agents", "benchmarks"], "SWE-bench, HumanEval, GAIA, agent leaderboards"),
        ]
    },
    "mcp-protocol": {
        "namespace": "mcp",
        "topics": [
            ("MCP Overview", ["mcp", "protocol"], "Model Context Protocol architecture, components, use cases"),
            ("MCP Server Creation", ["mcp", "server"], "server setup, TypeScript SDK, Python SDK, initialization"),
            ("MCP Tools", ["mcp", "tools"], "tool definitions, input schemas, tool handlers, error handling"),
            ("MCP Resources", ["mcp", "resources"], "resource providers, URIs, content types, subscriptions"),
            ("MCP Prompts", ["mcp", "prompts"], "prompt templates, arguments, dynamic prompts"),
            ("MCP Transport", ["mcp", "transport"], "stdio transport, SSE transport, WebSocket, HTTP"),
            ("MCP Claude Integration", ["mcp", "claude"], "Claude Desktop config, server registration, debugging"),
            ("MCP Filesystem Server", ["mcp", "filesystem"], "file operations, directory listing, search"),
            ("MCP Database Server", ["mcp", "database"], "SQL queries, schema introspection, transactions"),
            ("MCP GitHub Server", ["mcp", "github"], "repository operations, issues, PRs, code search"),
            ("MCP Brave Search", ["mcp", "search"], "web search integration, result formatting"),
            ("MCP Custom Tools", ["mcp", "custom"], "building custom tools, validation, async handlers"),
            ("MCP Error Handling", ["mcp", "errors"], "error types, error propagation, recovery strategies"),
            ("MCP Security", ["mcp", "security"], "authentication, authorization, sandboxing"),
            ("MCP Testing", ["mcp", "testing"], "unit tests, integration tests, mock clients"),
        ]
    },
    "debugging": {
        "namespace": "debugging",
        "topics": [
            ("HTTP Status Codes", ["http", "debugging"], "4xx errors, 5xx errors, common causes, resolution"),
            ("API Error Patterns", ["api", "errors"], "422 validation, 401/403 auth, 500 server, debugging flow"),
            ("Python Debugging", ["python", "debugging"], "pdb, breakpoint(), logging, traceback analysis"),
            ("JavaScript Debugging", ["javascript", "debugging"], "Chrome DevTools, console methods, source maps"),
            ("Database Query Debugging", ["database", "debugging"], "EXPLAIN plans, slow queries, connection issues"),
            ("Network Debugging", ["network", "debugging"], "curl, wget, tcpdump, Wireshark basics"),
            ("Container Debugging", ["docker", "debugging"], "logs, exec, inspect, health checks"),
            ("Kubernetes Debugging", ["kubernetes", "debugging"], "kubectl debug, logs, describe, events"),
            ("Memory Leak Detection", ["memory", "debugging"], "profiling, heap dumps, garbage collection"),
            ("Performance Profiling", ["performance", "profiling"], "CPU profiling, flame graphs, bottleneck identification"),
            ("Async Debugging", ["async", "debugging"], "promise rejection, async stack traces, race conditions"),
            ("Error Tracking", ["errors", "monitoring"], "Sentry, Rollbar, error aggregation, alerting"),
            ("Log Analysis", ["logging", "analysis"], "structured logging, log aggregation, search patterns"),
            ("Distributed Tracing", ["tracing", "debugging"], "OpenTelemetry, Jaeger, trace correlation"),
            ("Production Debugging", ["production", "debugging"], "safe debugging, feature flags, canary analysis"),
        ]
    },
    "llm-engineering": {
        "namespace": "llm-engineering",
        "topics": [
            ("Prompt Engineering Basics", ["prompts", "basics"], "zero-shot, few-shot, role prompting, formatting"),
            ("Chain of Thought", ["cot", "prompting"], "step-by-step reasoning, self-consistency, tree of thought"),
            ("Prompt Optimization", ["prompts", "optimization"], "prompt compression, token efficiency, caching"),
            ("Context Window Management", ["context", "llm"], "chunking strategies, summarization, sliding window"),
            ("RAG Architecture", ["rag", "architecture"], "retrieval pipeline, generation, hybrid approaches"),
            ("RAG Chunking Strategies", ["rag", "chunking"], "semantic chunking, recursive splitting, overlap"),
            ("RAG Retrieval Optimization", ["rag", "retrieval"], "hybrid search, reranking, query expansion"),
            ("RAG Evaluation Metrics", ["rag", "evaluation"], "faithfulness, relevancy, RAGAS framework"),
            ("LLM Caching Strategies", ["llm", "caching"], "semantic cache, KV cache, prompt caching"),
            ("LLM Output Parsing", ["llm", "parsing"], "structured outputs, JSON mode, retry strategies"),
            ("LLM Guardrails", ["llm", "safety"], "content filtering, output validation, jailbreak prevention"),
            ("LLM Cost Optimization", ["llm", "cost"], "model selection, batching, caching strategies"),
            ("LLM Evaluation", ["llm", "evaluation"], "benchmarks, human eval, automated metrics"),
            ("LLM Fine-tuning Basics", ["llm", "finetuning"], "when to fine-tune, dataset prep, evaluation"),
            ("LLM Deployment", ["llm", "deployment"], "inference servers, scaling, latency optimization"),
            ("Multimodal LLMs", ["multimodal", "llm"], "vision models, image understanding, document parsing"),
            ("Structured Generation", ["structured", "llm"], "JSON schema, Pydantic, instructor library"),
            ("LLM Streaming", ["streaming", "llm"], "SSE, token streaming, partial responses"),
            ("Model Selection", ["models", "selection"], "benchmarks, cost-performance, use case matching"),
            ("Local LLM Deployment", ["local", "llm"], "Ollama, llama.cpp, vLLM, MLX"),
        ]
    },
    "vector-search": {
        "namespace": "vector-search",
        "topics": [
            ("Vector Embeddings Fundamentals", ["embeddings", "basics"], "dense vectors, dimensions, similarity metrics"),
            ("Embedding Models Comparison", ["embeddings", "models"], "OpenAI, Cohere, sentence-transformers, benchmarks"),
            ("pgvector Setup", ["pgvector", "postgresql"], "installation, indexing, query optimization"),
            ("pgvector Advanced", ["pgvector", "advanced"], "HNSW vs IVFFlat, performance tuning, hybrid search"),
            ("Pinecone", ["pinecone", "vector-db"], "indexes, namespaces, metadata filtering, serverless"),
            ("Chroma", ["chroma", "vector-db"], "collections, embedding functions, persistence"),
            ("Weaviate", ["weaviate", "vector-db"], "schema, modules, GraphQL queries, hybrid search"),
            ("Qdrant", ["qdrant", "vector-db"], "collections, payload filtering, HNSW tuning"),
            ("Milvus", ["milvus", "vector-db"], "collections, indexes, distributed deployment"),
            ("Similarity Metrics", ["similarity", "metrics"], "cosine, euclidean, dot product, when to use"),
            ("Hybrid Search Implementation", ["hybrid", "search"], "BM25 + vector, RRF fusion, score normalization"),
            ("Reranking Strategies", ["reranking", "search"], "cross-encoders, ColBERT, Cohere rerank"),
            ("Semantic Search Optimization", ["semantic", "optimization"], "query expansion, synonyms, embedding fine-tuning"),
            ("Vector Index Types", ["indexes", "vector"], "HNSW, IVF, PQ, trade-offs"),
            ("Embedding Caching", ["embeddings", "caching"], "cache strategies, invalidation, batch processing"),
        ]
    },
    "api-design": {
        "namespace": "api-design",
        "topics": [
            ("REST API Design", ["rest", "api"], "resource naming, HTTP methods, status codes, HATEOAS"),
            ("OpenAPI Specification", ["openapi", "api"], "schema definition, documentation, code generation"),
            ("API Versioning", ["versioning", "api"], "URL path, headers, query params, deprecation"),
            ("API Error Handling", ["errors", "api"], "error schemas, problem details RFC, error codes"),
            ("API Rate Limiting", ["ratelimit", "api"], "algorithms, headers, client handling, quotas"),
            ("API Authentication", ["auth", "api"], "API keys, JWT, OAuth2, mTLS"),
            ("API Pagination", ["pagination", "api"], "offset, cursor, keyset, performance considerations"),
            ("API Caching", ["caching", "api"], "ETags, Cache-Control, CDN, conditional requests"),
            ("GraphQL Design", ["graphql", "api"], "schema design, resolvers, N+1 problem, federation"),
            ("gRPC Design", ["grpc", "api"], "protobuf, streaming, error handling, deadlines"),
            ("WebSocket API", ["websocket", "api"], "connection management, heartbeats, reconnection"),
            ("API Documentation", ["documentation", "api"], "Swagger UI, Redoc, examples, tutorials"),
            ("API Testing", ["testing", "api"], "contract testing, mocking, integration tests"),
            ("API Security", ["security", "api"], "OWASP API Top 10, input validation, CORS"),
            ("API Monitoring", ["monitoring", "api"], "metrics, logging, alerting, SLOs"),
        ]
    },
    "python-advanced": {
        "namespace": "python-advanced",
        "topics": [
            ("Python Async Deep Dive", ["python", "async"], "asyncio internals, event loop, task groups"),
            ("Python Decorators Advanced", ["python", "decorators"], "functools, class decorators, decorator factories"),
            ("Python Metaclasses", ["python", "metaclasses"], "type, __new__, __init__, use cases"),
            ("Python Descriptors", ["python", "descriptors"], "__get__, __set__, property implementation"),
            ("Python Context Managers", ["python", "context"], "contextlib, async context managers, resource management"),
            ("Python Type Hints Advanced", ["python", "typing"], "generics, protocols, TypeVar, overload"),
            ("Python Dataclasses", ["python", "dataclasses"], "field options, inheritance, frozen, slots"),
            ("Python Protocols", ["python", "protocols"], "structural subtyping, runtime checkable, duck typing"),
            ("Python Performance", ["python", "performance"], "profiling, optimization, Cython, numba"),
            ("Python Memory Management", ["python", "memory"], "gc, weakref, __slots__, memory profiling"),
            ("Python Packaging", ["python", "packaging"], "pyproject.toml, setuptools, poetry, uv"),
            ("Python Testing Patterns", ["python", "testing"], "pytest fixtures, mocking, parametrize"),
            ("Python Concurrency", ["python", "concurrency"], "threading, multiprocessing, concurrent.futures"),
            ("Python Metaprogramming", ["python", "metaprogramming"], "AST, code generation, inspect module"),
            ("Python Design Patterns", ["python", "patterns"], "singleton, factory, observer, strategy"),
        ]
    },
    "mlx-local-ai": {
        "namespace": "mlx",
        "topics": [
            ("MLX Fundamentals", ["mlx", "apple"], "arrays, operations, lazy evaluation, GPU acceleration"),
            ("MLX Model Loading", ["mlx", "models"], "from Hugging Face, quantization, memory mapping"),
            ("MLX Inference Server", ["mlx", "inference"], "server setup, API endpoints, batching"),
            ("MLX Fine-tuning", ["mlx", "finetuning"], "LoRA, QLoRA, dataset preparation, training"),
            ("MLX Quantization", ["mlx", "quantization"], "4-bit, 8-bit, mixed precision, quality trade-offs"),
            ("Ollama Setup", ["ollama", "local"], "installation, model pulling, customization"),
            ("Ollama API", ["ollama", "api"], "generate, chat, embeddings, model management"),
            ("Ollama Custom Models", ["ollama", "custom"], "Modelfile, system prompts, parameters"),
            ("llama.cpp", ["llamacpp", "local"], "compilation, GGUF format, server mode"),
            ("vLLM Serving", ["vllm", "serving"], "PagedAttention, continuous batching, throughput"),
            ("Local Model Selection", ["local", "models"], "quality vs speed, memory requirements, benchmarks"),
            ("Apple Silicon Optimization", ["apple", "silicon"], "Metal acceleration, unified memory, power efficiency"),
            ("Local AI Workflows", ["local", "workflows"], "offline usage, privacy, cost savings"),
            ("Model Conversion", ["models", "conversion"], "GGUF, safetensors, format compatibility"),
            ("Local Embeddings", ["embeddings", "local"], "nomic-embed, all-MiniLM, performance comparison"),
        ]
    },
}
