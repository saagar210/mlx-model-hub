...
# Start KAS API
echo "Starting KAS API on port 8000..."
if [ -d "$KAS_DIR" ]; then
    cd "$KAS_DIR"
    nohup uv run uvicorn knowledge.api.main:app --host 0.0.0.0 --port 8000 > /tmp/kas.log 2>&1 &
    echo "KAS PID: $!"
fi

# Start LocalCrew API
echo "Starting LocalCrew API on port 8001..."
LOCALCREW_DIR=~/claude-code/personal/crewai-automation-platform
if [ -d "$LOCALCREW_DIR" ]; then
    cd "$LOCALCREW_DIR"
    nohup uv run uvicorn localcrew.api.main:app --host 0.0.0.0 --port 8001 > /tmp/localcrew.log 2>&1 &
    echo "LocalCrew PID: $!"
fi

# Start UCE Dashboard
echo "Starting UCE Dashboard on port 8002..."
UCE_DIR=~/claude-code/projects-2026/end-of-january-projects/ai-native-dev-environment
if [ -d "$UCE_DIR" ]; then
    cd "$UCE_DIR"
    nohup uv run uvicorn universal_context_engine.dashboard.api:app --host 0.0.0.0 --port 8002 > /tmp/uce-dashboard.log 2>&1 &
